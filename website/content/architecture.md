...
两端均使用原生桌面界面和按需独立主面板；Windows v1.0.3 使用 C# / WPF，macOS v1.0.3 使用 Swift / AppKit，没有嵌入网页。两端的界面与系统集成分别维护，共享本地统计解析逻辑。

## macOS v1.0.2 起：原生监控与事件等待
...
下方 v1.0.1 进程与内存内容保留为历史设计背景，不作为 v1.0.2 新测量。当前实现见 [macOS 架构](https://github.com/Asher-XunZhang/codex-usage/blob/main/docs/macos/ARCHITECTURE.md)，本轮资源样本及边界见 [资源测量](https://github.com/Asher-XunZhang/codex-usage/blob/main/docs/macos/RESOURCE-MEASUREMENT.md)。

## Windows v1.0.3：托盘、浮窗与监控宿主

常驻宿主拥有托盘、浮窗、预算与任务监控；主面板按需启动自己的进程和本地 Python 服务。关闭主面板会释放这一部分，保留常驻入口与已选监控任务。

```mermaid
flowchart TB
  accTitle: Windows 桌面版的数据与提醒
  accDescr: 本地日志经共享 Python 索引写入 SQLite，Windows 宿主原生读取缓存，独立读取任务事件并记录提醒。主面板通过本地服务查询数据。
  L[本机 Codex 日志] --> P[按需 Python 增量索引]
  P --> DB[(本地 SQLite 缓存)]
  H[常驻 WPF 宿主：托盘与浮窗] --> N[Windows 原生 SQLite 读取]
  N --> DB
  M[独立 WPF 主面板] --> API[本地统计服务]
  API --> P
  H <-->|当前用户命名管道| M
  H --> T[任务事件读取与本地消息记录]
  T --> L
...
Windows 主面板通过有界后端事件与宿主状态推送更新，HTTP 同时等待请求与停止 socket；正常运行不再定时查询健康状态，异常时才进入恢复重试。Windows 的父进程监测同时等待进程退出与停止事件，避免用固定超时反复唤醒这条等待线程。圆环形变和回位动画在过渡结束后停止渲染回调；贴边与任务监测仍有各自的活动检查，不能据此声称整个程序零轮询或零闲置成本。

进程使用当前用户命名管道与 Windows 进程管理；主面板服务仅监听本机。具体实现位于 `src/windows/Infrastructure/` 和 `src/backend/`，工程入口见[开发指南](./development.md)。
