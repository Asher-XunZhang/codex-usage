...
以下工具仅供 macOS 开发与检查使用。新入口的默认预览位于 `build/previews/macos/`，采样报告位于 `build/reports/macos/`，均忽略提交。旧 `scripts/` 兼容入口仍使用原有 `.local/` 默认目录。

## 离屏界面预览

```sh
python3 -m tools.macos.render_previews
python3 -m tools.macos.render_previews --source src/macos/Features/Floating/Capsule.swift --output .local/previews
```

需要 Apple Command Line Tools。脚本编译当前 `Capsule.swift` 与 `tools/macos/preview/main.swift`，生成深浅主题、额度边界、长文本和展开中间帧的 26 张 PNG，以及一张收起样式合集。全部数字、时间、模型及任务均为合成示例，不读取本机 Codex 日志、账号或应用偏好，不打开已安装应用或可见窗口。
...
```sh
python3 -m tools.macos.render_previews --output .local/previews
...
```sh
python3 -m tools.macos.measure_processes --duration 40
python3 -m tools.macos.measure_processes --app '/Applications/Codex用量.app' --duration 70 --output .local/performance
```
