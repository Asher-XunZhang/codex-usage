...
| 构建与分发 | 独立 x64 ZIP、自包含 .NET 与固定校验 CPython 3.14.7；中文／空格路径离线验证，源码及分发文件哈希，源变更要求重建；v1.0.2 Windows x64 便携包未做代码签名。 | 同名 Release 补充 macOS 两架构包、签名完整性检查、固定摘要安装器与编译来源清单，保留原 Windows 资产和标签。 |
