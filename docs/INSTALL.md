...
当前发行版为 **v1.0.0**，使用 ad hoc 签名，未经过 Developer ID 签名或 Apple 公证。免费安装助手减少首次安装的手工步骤；它不能赋予 Apple 公证信任，也不能保证绕过组织管理策略。用户应先确认信任本仓库和本次发布。

## 安装助手

### 在线安装

在 macOS「终端」执行 [README 中的安装命令](../README.md#自动选择芯片并安装推荐)。命令从本仓库下载 `scripts/install.sh`，保存到临时目录，再用系统 `/bin/bash` 执行。也可先查看 [脚本源码](https://github.com/Asher-XunZhang/codex-usage/blob/main/scripts/install.sh)。

助手会：

1. 检测实际芯片并选择 Intel 或 Apple Silicon 发布包；在 Rosetta 终端中也会选择 Apple Silicon 版本。
2. 展示固定版本、来源、目标路径和隔离处理说明，要求在交互终端输入 `install` 确认。
3. 下载固定的 v1.0.0 ZIP，核对脚本中固定的 SHA-256，解压后检查 App 的签名完整性。
...
README 的在线脚本地址指向 `main`，PR 合并后才包含该 PR 的改动。评审尚未合并的 PR 时，在该分支仓库根目录运行：
...
脚本仍安装固定的 v1.0.0 发行 App，并不编译当前分支；当前源码可能已有尚未发布的功能。

### 离线安装

提前取得 `scripts/install.sh` 和 [v1.0.0 发行页](https://github.com/Asher-XunZhang/codex-usage/releases/tag/v1.0.0)中与芯片匹配的完整 ZIP。把下例路径换成实际文件路径：
...
Apple Silicon 使用文件名以 `AppleSilicon.zip` 结尾的资产。离线模式同样校验固定摘要，只接受当前脚本对应架构的 v1.0.0 发布包；不能用于任意 ZIP、修改过的 App 或 GitHub 的 Source code 包。

| 架构 | v1.0.0 ZIP 的 SHA-256 |
| --- | --- |
...
1. 在「 → 关于本机」查看芯片，使用 Safari 从 [正式发行页](https://github.com/Asher-XunZhang/codex-usage/releases/tag/v1.0.0)下载对应 ZIP。
