...
from tools.windows.build import SDK_VERSION, version
from tools.common.downloads import digest_file
from tools.windows.package import check_path, validate_app, windows_sources, verify_pe_x64, release_readme
...
def verify_documentation_images(root):
    ...
    require((root / 'README.md').read_bytes() == release_readme(), 'Unexpected Windows release README or documentation entry points')
    require((root / 'source/docs/README.md').is_file(), 'Missing platform documentation index')
    require((root / 'source/docs/windows/README.md').is_file(), 'Missing Windows documentation entry point')
    documentation_root = root / 'source/docs'
    documents = sorted(documentation_root.rglob('*.md'))
    for document in documents:
        ...
        if name == 'source/docs/windows/README.md':
            require(bool(references), f'Windows documentation is missing its preview images: {name}')
        ...
        # Check the reorganized documentation graph without broadening this check
        # to source-code/dev-tool links outside the published documentation tree.
        for reference in re.findall(r'\[[^\]]*\]\(\s*(<[^>]+>|[^\s)]+)', markup):
            url = urlsplit(reference.strip('<>'))
            if url.scheme or url.netloc or not url.path:
                continue
            target = (document.parent / unquote(url.path)).resolve()
            if target.is_relative_to(documentation_root) and target.suffix in {'.md', '.png', '.json'}:
                require(target.is_file(), f'Broken documentation link: {name} -> {reference}')
...
def inspect_archive(path, destination):
    ...
    with zipfile.ZipFile(path) as archive:
        ...
        allowed_extra = {'README.md', 'LICENSE', 'SHA256SUMS.json'}
        ...
        for name in manifest['files']:
            if name.startswith('backend/'):
                require(digest_file(destination / name) == digest_file(destination / 'source/src' / name),
...
def main(*, legacy=False):
    ...
    parser.add_argument('--archive', type=Path, default=ROOT / ('dist' if legacy else 'dist/windows') / f'codex-usage-desktop-v{version()}-windows-x64.zip')
