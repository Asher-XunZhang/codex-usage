...
class BudgetCoordinatorTests(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        host_source = (macos_source('BudgetHost.swift')).read_text()
        ...
        compiled = subprocess.run(['xcrun', 'swiftc', '-O', str(macos_source('BudgetCore.swift')), str(harness), '-o', str(cls.binary)], capture_output=True, text=True)
