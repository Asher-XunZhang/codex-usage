...
更新日期：2026-09-19。用途：保留 Windows 已落地的行为编号、实现入口与历史效果图；macOS 最新完成状态统一见[当前适配清单](MACOS-ADAPTATION-BACKLOG.md)，不再从历史基线推断。

## 1. 比较基线与状态

- **历史 macOS 比较基线**：`f7f35c964bca7770405b169bdd3104f5a4c8e002`，已含预算源码，不能等同于公开 v1.0.1 安装包。
- **macOS 对照**：v1.0.3，发行源码为 `e676b60c391f69f4d2d47b885f2e08271670551a`。数据、预算保护、任务监控、四方向展开、全区域拖动、微弧侧签、弧线调色和事件等待均已实现；[发行说明](../macos/RELEASE-1.0.3.md)列出产物来源和验证边界。
- **Windows 对照**：v1.0.3，基于 `aaafa16` 完成本轮对齐。既有同名标签仍对应 macOS 首发源码，Windows 源码提交见 Release 说明及包内 `source/`。第 8 节图片保留内部 1.3.1 快照，仅说明当时布局。2026-09-19 已补齐弧线配色、微弧侧签及动态尺寸、紧凑未读数、额度读取开关、通知关联消息与暂停操作、365 天历史选项；接缝与事件等待的依据见[本轮对齐决策](../windows/MACOS-PARITY-DECISIONS.md)。
...
| F06 | **四边吸附与自动隐藏**：靠近工作区边缘 16 DIP 松开可吸附。两端均允许相邻屏幕接缝贴合并自动收成侧签，以松手指针所在屏幕为归属，侧签留在该屏幕内。菜单、拖动和保持展开期间暂停隐藏。 |
| F07 | **额度侧签**：微弧融合轮廓，无活跃任务及未读时左右 44×68 DIP、上下 88×28 DIP；有监控信息时连续扩至 44×96 或 124×28 DIP。显示当前账号额度或所选预算的剩余／已用百分比，未读数字最多显示 99+。未知显示“—”，过期有标记；默认警示色仍由剩余额度决定，文字及读屏语义一致。 |
...
| --- | --- | --- |
| 原生 UI | C#／WPF + Win32 托盘，无 WebView；`.NET 10`，公开版本 1.0.3。 | 保留 Swift／AppKit，复用第 3 节行为规范。 |
...
| 构建与分发 | 独立 x64 ZIP、自包含 .NET 与固定校验 CPython 3.14.7；中文／空格路径离线验证，源码及分发文件哈希，源变更要求重建；v1.0.3 Windows x64 便携包未做代码签名。 | 同名 Release 提供 macOS 两架构包、签名完整性检查、固定摘要安装器与编译来源清单；各平台资产和源码来源分别记录。 |
...
以下列出共享源码的当前处理。源码更新不会替换已安装 App，各平台发行资产的源码来源和验证范围以对应 Release 为准。
...
| 空闲服务与刷新等待 | macOS HTTP 循环等待请求或停止管道；Windows 使用监听 socket 与停止 socket 等待，已移除 0.25 秒空闲超时。Windows 后端状态与刷新完成通过有界 stdout 事件传递，主面板接收宿主带版本的 IPC 推送；异常时才定时恢复。共享索引等待实际刷新期限，关闭自动刷新时无限等待。 | 核对手动刷新、设置切换、退出信号、乱序结果和待完成刷新请求；见 [Windows 对齐决策](../windows/MACOS-PARITY-DECISIONS.md)与 [macOS 空闲等待优化](../macos/IDLE-WAITS.md)。 |
...
- [Windows 任务监控](../windows/TASK-MONITOR.md)和 [macOS 任务监控](../macos/TASK-MONITOR.md)均已随各自 v1.0.2 实现。旧截图不包含新增监控内容；macOS 系统通知已确认展示，真实正文点击／冷启动路径尚未全部验收，不将 API 成功当作用户已收到。
...
对应 **F04–F08**。以下为历史图片，从左到右为左侧、右侧、顶部和底部侧签，按 **2 倍逻辑尺寸**显示。当时左右为 28×72 DIP、上下为 76×28 DIP；当前微弧轮廓与动态尺寸见 F07。数值只表达当前所选额度，未知用“—”，小状态标记表示旧记录；侧签附近局部停留先唤回圆环，不直接展开整个详情框。
