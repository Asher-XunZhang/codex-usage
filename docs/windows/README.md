...
从 [v1.0.2 发布页](https://github.com/Asher-XunZhang/codex-usage/releases/tag/v1.0.2)下载 `codex-usage-desktop-v1.0.2-windows-x64.zip`，完整解压到有写入权限的目录，双击 `CodexUsage.exe`。保留同目录的 DLL、`python/` 和 `backend/`；不能只复制 EXE。
...
拖动时可超出屏幕，松手后越界面板平滑回到可见范围，内部位置不会强制吸边。启用贴边隐藏后，可贴到显示器有效外边缘；面板回位时保留圆环的贴边位置。下次展开按当前位置选择向左或向右、向上或向下，不固定使用上次方向。

贴边后鼠标离开会隐藏为小侧签，显示剩余或已用比例，未知时显示“—”；任务状态用独立符号表达。停留在侧签附近先唤回圆环，继续移入圆环才展开。相邻显示器连接处不作为隐藏边缘。菜单、拖动或保持展开期间暂停自动隐藏；关闭系统动画时直接显示终态。
