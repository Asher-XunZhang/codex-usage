...
usage() {
    cat <<'EOF'
Codex 用量快捷安装（固定 v1.0.0）
用法：/bin/bash scripts/install.sh [选项]
  --archive ZIP       使用已经下载的官方 v1.0.0 ZIP，仍校验固定 SHA-256
...
select_release() {
    ...
    case "$architecture" in
        arm64)
            label=AppleSilicon
            expected_sha=18bad090d4dd52a4d31177fe8cca0be9d52dc8e165650765a16cc6110b29c220
            ;;
        x86_64)
            label=Intel
            expected_sha=4dbcd5b12cd4c96580b223dafdb612dc6f2a268913923e8a5f788278dba0e4d2
            ;;
