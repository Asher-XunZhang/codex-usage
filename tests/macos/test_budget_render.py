...
class BudgetRenderTests(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        host = (macos_source('BudgetHost.swift')).read_text()
        quota = (macos_source('Quota.swift')).read_text()
        ...
        result = subprocess.run(['xcrun', 'swiftc', '-O', str(macos_source('Capsule.swift')), str(harness),
