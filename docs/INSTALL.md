...
在 macOS「终端」执行 [README 中的安装命令](../README.md#自动选择芯片并安装推荐)。命令从 v1.0.1 Release 下载独立的 `install.sh`，保存到临时目录，再用系统 `/bin/bash` 执行。也可先查看 [脚本源码](https://github.com/Asher-XunZhang/codex-usage/blob/main/scripts/install.sh)。
