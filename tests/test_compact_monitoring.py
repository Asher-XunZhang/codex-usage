...
'''
        ...
        built = subprocess.run(['xcrun', 'swiftc', '-swift-version', '5', str(sources / 'UsageChangeMonitor.swift'), str(swift), '-o', str(cls.binary)], capture_output=True, text=True, timeout=90)
        if built.returncode:
