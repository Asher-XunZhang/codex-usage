...
from scripts.build_windows import BACKEND_FILES, SDK_VERSION, build_sources, regular_files, version
...
def check_path(name):
    ...
    metadata = name in ('.github/workflows/windows.yml', 'source/.github/workflows/windows.yml', f'CodexUsage-{version()}-windows-x64/source/.github/workflows/windows.yml')
...
def validate_app(root, check_sources=True, extra_files=()):
    ...
    if 'BUILD-MANIFEST.json' not in files:
        raise ValueError('Missing BUILD-MANIFEST.json; run scripts/build_windows.py first')
    ...
    if check_sources:
        ...
        for name in BACKEND_FILES:
            if digest_file(ROOT / 'backend' / name) != digest_file(root / 'backend' / name):
...
def documentation_images(root=ROOT):
    return sorted((root / 'docs/images').glob('windows-*.png'))
...
def package(app, output_dir):
    ...
    payload['README.md'] = ROOT / 'docs/WINDOWS.md'
    payload['LICENSE'] = ROOT / 'LICENSE'
    payload['docs/WINDOWS.md'] = ROOT / 'docs/WINDOWS.md'
    # WINDOWS.md is also the ZIP README; keep its relative image links valid in both places.
    for image in documentation_images():
        payload['images/' + image.name] = image
        payload['docs/images/' + image.name] = image
    payload.update({'source/' + p.relative_to(ROOT).as_posix(): p for p in windows_sources()})
    ci_recipe = ROOT / '.github/workflows/windows.yml'
    if not ci_recipe.is_file():
        ci_recipe = ROOT / 'windows-ci.yml'  # Repackaging the published source tree.
    payload['source/windows-ci.yml'] = ci_recipe
    hashes = {}
    seen = set()
...
def main():
    ...
    parser.add_argument('--app', type=Path, default=ROOT / 'build/windows-x64')
