...
class FloatingChoicesReaderTests(unittest.TestCase):
    def test_native_large_reply_replacement_cancel_and_callback_release(self):
        reader = declaration((macos_source('Runtime.swift')).read_text(), 'final class FloatingChoicesReader')
        summary_reader = declaration((macos_source('Runtime.swift')).read_text(), 'final class NativeSummaryReader')
        query = declaration((macos_source('Main.swift')).read_text(), 'struct FloatingUsageQuery:')
...
''')
            ...
            compiled = subprocess.run(['xcrun', 'clang', '-Os', str(macos_source('Summary.c')), '-lsqlite3', '-o', str(root / 'CodexSummary')], capture_output=True, text=True, timeout=30)
